...
export class Game {
  constructor() {
    ...
    this.bossDefeatedTracked = false;
      ...
      for (const enemy of this.level.enemies) {
        if (enemy.active && projectile.checkCollision(enemy)) {
          ...
          if (enemy instanceof CriticalStakeholder && enemy.health <= 0) {
            // Track boss defeat (guarded to fire only once per game session)
            if (!this.bossDefeatedTracked && typeof pendo !== "undefined") {
              this.bossDefeatedTracked = true;
              pendo.track("boss_defeated", {
                ...
                calls_remaining: this.player.callAmmo,
                emails_remaining: this.player.emailAmmo,
              });
            }
  ...
  gameOver(victory) {
    ...
    if (victory) {
      ...
      // Track victory completion
      const boss = this.level.enemies.find(
        (e) => e instanceof CriticalStakeholder,
      );
      const bossDefeated = boss ? !boss.active : true;
      ...
      if (typeof pendo !== "undefined") {
        pendo.track("game_completed_defeat", {
          final_score: this.score,
          player_x_position: this.player.x,
          player_health_at_death: this.player.health,
          calls_remaining: this.player.callAmmo,
          emails_remaining: this.player.emailAmmo,
        });
      }
  ...
  restart() {
    // Capture previous state for tracking before resetting
    const previousScore = this.score;
    const previousOutcome = this.lastVictory ? "victory" : "defeat";
    const boss = this.level.enemies.find(
      (e) => e instanceof CriticalStakeholder,
    );
    const previousBossDefeated = boss ? !boss.active : true;
    ...
    if (typeof pendo !== "undefined") {
      pendo.track("game_restarted", {
        ...
        previous_boss_defeated: previousBossDefeated,
      });
    }
