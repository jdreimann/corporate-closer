...
export class Game {
      ...
      for (const enemy of this.level.enemies) {
        if (enemy.active && projectile.checkCollision(enemy)) {
          ...
          this.audioManager.playSound(
            enemy.health <= 0 ? "enemyDestroy" : "enemyHit",
          );
          projectile.hit();

          // Track defeat events only on the killing blow
          if (
            healthBefore > 0 &&
            enemy.health <= 0 &&
            typeof pendo !== "undefined"
          ) {
            if (enemy instanceof CriticalStakeholder) {
              pendo.track("boss_defeated", {
                ...
                player_call_ammo_remaining: this.player.callAmmo,
                player_email_ammo_remaining: this.player.emailAmmo,
              });
            } else {
              pendo.track("enemy_defeated", {
                enemy_type:
                  enemy instanceof MeetingDecline
                    ? "MeetingDecline"
                    : "FinanceReview",
                weapon_type:
                  projectile instanceof EmailProjectile ? "email" : "call",
                enemy_score_value: enemy.scoreValue,
  ...
  checkVictoryCondition() {
    ...
    if (this.player.x > this.level.width - 100) {
      ...
      const boss = this.level.enemies.find(
        (e) => e instanceof CriticalStakeholder,
      );
      if (boss && !boss.active) {
  ...
  gameOver(victory) {
    // Track game completion (guard against duplicate calls)
    if (this.gameState !== "gameOver" && typeof pendo !== "undefined") {
      const boss = this.level.enemies.find(
        (e) => e instanceof CriticalStakeholder,
      );
      pendo.track("game_completed", {
        ...
        player_health_remaining: this.player.health,
        player_x_position: Math.round(this.player.x),
        boss_defeated: !boss || boss.health <= 0,
        enemies_remaining_count: this.level.getActiveEnemies().length,
        call_ammo_remaining: this.player.callAmmo,
        email_ammo_remaining: this.player.emailAmmo,
      });
    }
    ...
    if (victory) {
      ...
      this.gameOverMessage.textContent =
        "Congratulations! You've successfully navigated the corporate maze and closed the deal. Your sales skills are unmatched!";
      this.audioManager.playSound("victory");
    } else {
      this.gameOverTitle.textContent = "Deal Lost";
      this.gameOverMessage.textContent =
        "The corporate world got the better of you this time. Don't give up - every great salesperson faces rejection!";
      ...
      pendo.track("game_restarted", {
        previous_outcome:
          this.gameOverTitle.textContent === "Deal Closed!"
            ? "victory"
            : "defeat",
        previous_final_score: this.score,
      });
    }
  ...
  drawHUD() {
    ...
    const boss = this.level.enemies.find(
      (e) => e instanceof CriticalStakeholder && e.active,
    );
    if (boss) {
      // Check if boss is in camera view
      const bossInView =
        boss.x >= this.engine.camera.x - 100 &&
      ...
      if (bossInView && !this.bossFirstSeen) {
        ...
        if (typeof pendo !== "undefined") {
          pendo.track("boss_encountered", {
            player_health: this.player.health,
            player_score: this.score,
            player_call_ammo: this.player.callAmmo,
            player_email_ammo: this.player.emailAmmo,
            player_x_position: Math.round(this.player.x),
          });
        }
      }
